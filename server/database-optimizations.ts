...
import { User, Employee, AttendanceRecord, LeaveRequest, Project, Task, Department } from './db';
...
export class DatabaseOptimizer {
  ...
  // Search optimization
  async searchEmployees(query: string, limit: number = 20) {
    return await this.sequelize.query(`
      SELECT 
        e.id,
  ...
  async bulkUpdateAttendance(records: Array<{id: string, status: string, clockOut?: string}>) {
    ...
    `, {
      replacements: updates.reduce((acc: any, [id, status, clockOut], i) => {
        acc[`id${i}`] = id;
        acc[`status${i}`] = status;
